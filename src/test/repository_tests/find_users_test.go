...
func TestUserRepository_FindUserByEmailAndPassword(t *testing.T) {
	...
	mtestDb.Run("when_sending_a_valid_email_and_password_returns_success", func(mt *mtest.T) {
		...
		repo := repository.NewUserRepository(databaseMock)
		userDomain, err := repo.FindUserByEmailAndPass(userEntity.Email, userEntity.Password)
	...
	mtestDb.Run("returns_error_when_mongodb_returns_error", func(mt *mtest.T) {
		...
		repo := repository.NewUserRepository(databaseMock)
		userDomain, err := repo.FindUserByEmailAndPass("test", "testpass")
		assert.NotNil(t, err)
		assert.Nil(t, userDomain)
	})
...
func TestUserRepository_FindUserById(t *testing.T) {
	...
	mtestDb.Run("returns_no_document_found", func(mt *mtest.T) {
		...
		assert.NotNil(t, err)
		assert.Equal(t, err.Message, fmt.Sprintf("User with id test not found"))
		assert.Nil(t, userDomain)
	})
}

func convertEntityToBson(userEntity entities.UserEntityStruct) bson.D {
	return bson.D{
